...
from bot.state_router import (
    determine_state, format_sc_wallet_error,
    NO_ACCOUNT, NO_IDENTITY, IN_GAME, READY_PAID, READY_FREE, SC_WALLET_BLOCKED
)
...
from bot.game.ws_join import join_via_websocket, JoinError  # v1.6.0 unified join
...
from bot.config import (
    ...
    USE_V160_JOIN,  # v1.6.0 feature flag
)
...
class Heartbeat:
    ...
    async def _heartbeat_cycle(self):
        ...
        if state == SC_WALLET_BLOCKED:
            # v1.6.0: SC Wallet policy violation
            await self._handle_sc_wallet_blocked(me, ctx)
            return
    ...
    async def _handle_sc_wallet_blocked(self, me: dict, ctx: dict):
        """
        v1.6.0: Handle SC Wallet Policy violations.
        Per sc-wallet-policy.md:
        - NOT_PRIMARY_AGENT: agent not primary for SC wallet
        - ACTIVE_FREE_GAME_EXISTS: another agent in free game
        - ACTIVE_PAID_GAME_EXISTS: another agent in paid game
        """
        issues = ctx.get("issues", [])
        error_msg = format_sc_wallet_error(issues)
        
        log.error("╔════════════════════════════════════════════════════════╗")
        log.error("║   SC WALLET POLICY VIOLATION - Cannot Join Game      ║")
        log.error("╚════════════════════════════════════════════════════════╝")
        log.error("Reason: %s", error_msg)
        
        # Check if we should notify owner (first time only)
        from bot.memory.agent_memory import AgentMemory
        memory = AgentMemory()
        await memory.load()
        
        notified_key = "sc_wallet_violation_notified"
        last_notified = memory.get_metadata(notified_key)
        
        if not last_notified:
            # First violation - notify owner
            log.error("📧 Owner notification required!")
            log.error("Action: Check My Agent page and verify primary agent status.")
            
            # Save notification timestamp
            memory.set_metadata(notified_key, True)
            await memory.save()
        
        # Wait longer before retry (5 minutes)
        # This gives owner time to fix or wait for other games to finish
    ...
    async def _handle_ready(self, me: dict, state: str):
        ...
        # v1.6.0: Use unified WebSocket join when enabled
        if USE_V160_JOIN:
            await self._handle_ready_v160(me, state, room_type)
        else:
            await self._handle_ready_legacy(me, state, room_type)
    
    async def _handle_ready_v160(self, me: dict, state: str, room_type: str):
        """v1.6.0 unified WebSocket join via /ws/join"""
        try:
            if room_type == "paid":
                # Check balance before attempting paid join
                balance = me.get("balance", 0)
                if balance < 500:
                    log.warning("Insufficient sMoltz: %s/500. Falling back to free.", balance)
                    room_type = "free"
                    game_id, agent_id, ws = await join_via_websocket(self.api, "free")
                else:
                    game_id, agent_id, ws = await join_via_websocket(self.api, "paid", "offchain")
            else:
                game_id, agent_id, ws = await join_via_websocket(self.api, "free")
            
            # Successfully joined → play with existing socket
            ...
            if e.code == "NO_IDENTITY":
                ...
                return
            elif e.code in ("NOT_PRIMARY_AGENT", "ACTIVE_FREE_GAME_EXISTS", "ACTIVE_PAID_GAME_EXISTS"):
                log.error("SC Wallet policy violation: %s. Notify owner.", e.code)
                # TODO: Notify owner about SC wallet policy
                await asyncio.sleep(60)
                return
            log.warning("v1.6.0 Join failed: %s (code=%s). Retrying in 10s.", e, e.code)
            await asyncio.sleep(10)
            return
        except Exception as e:
            log.warning("v1.6.0 Join failed: %s. Retrying in 10s.", e)
            await asyncio.sleep(10)
            return
    
    async def _handle_ready_legacy(self, me: dict, state: str, room_type: str):
        """Legacy v1.5.2 join flow (HTTP + separate WS)"""
        try:
            if room_type == "paid":
    ...
    async def _play_game(self, game_id: str, agent_id: str, entry_type: str, existing_ws=None):
        ...
        # v1.6.0: Pass existing socket from unified join (if available)
