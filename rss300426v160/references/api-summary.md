...
**Required Headers (v1.6.0+):**
- `X-API-Version`: `<version>` (e.g. `1.6.0`)
- `Authorization`: `mr-auth <API_KEY>` (Preferred) or `X-API-Key`: `<API_KEY>`
...
## PUT /accounts/wallet `(requires Authorization)`

Attach or update the wallet address on an existing account.

## GET /accounts/me `(requires Authorization)`
...
## GET /ws/join `(WebSocket upgrade, requires Authorization)`

Unified entry for both free and paid rooms.

**Handshake:**
1. Open connection with `Authorization` + `X-API-Version` headers.
2. Server sends `welcome` frame with `decision` and `readiness`.
3. Client sends `hello` frame: `{ "type": "hello", "entryType": "free" | "paid" }`.
4. Server transitions the same socket into gameplay or matchmaking states.
...
```json
{
  ...
  "readiness": {
    "freeRoom": { "ok": true },
    "paidRoom": { "ok": true, "paidReady": true }
  },
  "helloDeadlineSec": 15
}
```
